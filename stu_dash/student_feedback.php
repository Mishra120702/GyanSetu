...
try {
    ...
    $current_batches = [];
    $batch_fields = ['batch_name', 'batch_name_2', 'batch_name_3', 'batch_name_4', 'batch_name_5', 'batch_name_6', 'batch_name_7', 'batch_name_8', 'batch_name_9', 'batch_name_10'];
    ...
    if (!empty($current_batches) && isset($current_batches[$selected_batch_index])) {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit_feedback'])) {
            ...
            if ($valid) {
                ...
                if ($result) {
                    logSystemActivity($db, $student_id, 'FEEDBACK_SUBMITTED', "Student $student_name submitted feedback for course " . ($student['course_name'] ?? 'Unknown Course') . " in batch $selected_batch_id.");
        ...
        // Get trainer's weekly feedback for the student for the selected batch
        $trainer_feedback_query = $db->prepare("
            SELECT wf.*, 
                   COALESCE(t.name, 'Trainer') as trainer_name
            FROM weekly_feedback wf
            LEFT JOIN trainers t ON (wf.trainer_id = t.id OR wf.trainer_id = t.user_id)
            WHERE wf.student_id = :student_id AND wf.batch_id = :batch_id
            ORDER BY wf.week_start_date DESC
        ");
        $trainer_feedback_query->execute([
            ':student_id' => $student['student_id'],
            ':batch_id' => $selected_batch_id
        ]);
        $trainer_feedback_history = $trainer_feedback_query->fetchAll(PDO::FETCH_ASSOC);
    }
        ...
        <div class="mb-6 glass-card p-4">
            ...
            <div class="flex flex-wrap gap-2">
                <?php foreach ($current_batches as $index => $batch_info): 
                    ...
                    else $batch_label .= str_replace('batch_name_', '', $batch_info['field_name']);
            ...
            <?php if ($selected_batch): ?>
                <p class="text-sm text-gray-500 mt-2">
                    Currently viewing: <span class="font-semibold text-indigo-600">
                        ...
                        else $current_label .= str_replace('batch_name_', '', $current_batches[$selected_batch_index]['field_name']);
        ...
        <div class="bg-white p-6 rounded-2xl shadow-lg mt-6 transform transition-transform duration-300 hover:scale-[1.005]">
            <div class="flex justify-between items-center mb-4">
                ...
                <?php if ($selected_batch): ?>
                    <span class="text-sm font-medium text-gray-600">
                        For: <span class="text-indigo-600"><?= htmlspecialchars($selected_batch['batch_name']) ?></span>
                        (<?= count($trainer_feedback_history) ?> reviews)
                    </span>
                <?php endif; ?>
            </div>
            
            <?php if (count($trainer_feedback_history) > 0): ?>
                <div class="space-y-4">
                    <?php foreach ($trainer_feedback_history as $t_feedback): ?>
                        <div class="border border-gray-200 rounded-lg p-4 transition-all duration-300 hover:border-indigo-300 hover:shadow-md feedback-card transform hover:-translate-y-1">
                            <div class="flex justify-between items-start">
                                <div>
                                    <span class="text-gray-600 text-sm font-medium bg-indigo-50 px-2 py-1 rounded-full">
                                        <i class="far fa-calendar-alt mr-1"></i>
                                        Week: <?= date('M j', strtotime($t_feedback['week_start_date'])) ?> - <?= date('M j, Y', strtotime($t_feedback['week_end_date'])) ?>
                                    </span>
                                    <span class="ml-2 text-sm text-gray-500">
                                        <i class="fas fa-user-tie mr-1"></i>
                                        Trainer: <?= htmlspecialchars($t_feedback['trainer_name']) ?>
                                    </span>
                                </div>
                                <span class="text-sm font-bold px-2 py-1 rounded-full <?= $t_feedback['rating'] >= 4 ? 'bg-green-100 text-green-800' : ($t_feedback['rating'] == 3 ? 'bg-yellow-100 text-yellow-800' : 'bg-red-100 text-red-800') ?>">
                                    Rating: <?= htmlspecialchars($t_feedback['rating']) ?>/5
                                </span>
                            </div>
                            <div class="mt-3 bg-gray-50 p-3 rounded-lg transition-all duration-300 hover:bg-indigo-50">
                                <?php if (!empty($t_feedback['remarks'])): ?>
                                    <p class="text-sm font-medium text-gray-800 flex items-center">
                                        <i class="fas fa-comment mr-1 text-indigo-500"></i>
                                        Trainer Comments:
                                    </p>
                                    <p class="text-sm text-gray-700 mt-1"><?= nl2br(htmlspecialchars($t_feedback['remarks'])) ?></p>
                                <?php else: ?>
                                    <p class="text-sm text-gray-500 italic">No comments provided.</p>
                                <?php endif; ?>
                            </div>
                            <div class="mt-2 text-xs text-gray-500 text-right">
                                Submitted on: <?= date('M j, Y g:i A', strtotime($t_feedback['submitted_at'])) ?>
                            </div>
                        </div>
                    <?php endforeach; ?>
                </div>
            <?php else: ?>
                <div class="text-center py-8">
                    <div class="bg-indigo-50 inline-block p-4 rounded-full mb-4">
                        <i class="fas fa-chalkboard-teacher text-indigo-500 text-3xl"></i>
                    </div>
                    <p class="text-gray-500 text-lg">
                        <?php if ($selected_batch): ?>
                            No weekly feedback from your trainer for <?= htmlspecialchars($selected_batch['batch_name']) ?> yet.
                        <?php else: ?>
                            No batch selected.
                        <?php endif; ?>
                    </p>
...
<style>
    ...
    /* Dynamic badge classes for ratings */
    .rating-badge-poor {
    ...
    div[class*="from-blue-50"][class*="to-indigo-50"] {
        background: linear-gradient(135deg, rgba(27, 60, 83, 0.06) 0%, rgba(210, 193, 182, 0.15) 100%) !important;
    ...
    .text-indigo-600 { color: #234C6A !important; }
    .bg-indigo-100   { background: rgba(210, 193, 182, 0.4) !important; }
    .text-indigo-800 { color: #1B3C53 !important; }
    .text-indigo-300 { color: #456882 !important; }
    ...
    button[type="submit"], .submit-btn, button[name="submit_feedback"] {
    ...
    button[type="submit"]:hover, .submit-btn:hover, button[name="submit_feedback"]:hover {
        background: linear-gradient(135deg, #234C6A, #1B3C53) !important;
        transform: translateY(-2px) !important;
        box-shadow: 0 6px 20px rgba(27, 60, 83, 0.4) !important;
    }

    /* --- Premium Overview Cards & Containers --- */
    div.bg-white.p-6.rounded-2xl.shadow-lg {
        background: rgba(255, 255, 255, 0.75) !important;
        backdrop-filter: blur(12px) !important;
        -webkit-backdrop-filter: blur(12px) !important;
        border: 1px solid rgba(69, 104, 130, 0.25) !important;
        box-shadow: 0 8px 32px rgba(27, 60, 83, 0.06) !important;
        border-radius: 20px !important;
        transition: all 0.3s cubic-bezier(0.4, 0, 0.2, 1) !important;
    }
    div.bg-white.p-6.rounded-2xl.shadow-lg:hover {
        transform: translateY(-4px) scale(1.008) !important;
        box-shadow: 0 16px 36px rgba(27, 60, 83, 0.12) !important;
        border-color: rgba(69, 104, 130, 0.4) !important;
        background: rgba(255, 255, 255, 0.8) !important;
    }

    /* Card Icon Containers */
    div.bg-white.p-6.rounded-2xl.shadow-lg .bg-blue-100,
    div.bg-white.p-6.rounded-2xl.shadow-lg .bg-green-100,
    div.bg-white.p-6.rounded-2xl.shadow-lg .bg-purple-100 {
        background: linear-gradient(135deg, rgba(27, 60, 83, 0.1) 0%, rgba(210, 193, 182, 0.2) 100%) !important;
        border-radius: 12px !important;
        padding: 10px !important;
        border: 1px solid rgba(69, 104, 130, 0.2) !important;
    }
    div.bg-white.p-6.rounded-2xl.shadow-lg i.text-blue-600,
    div.bg-white.p-6.rounded-2xl.shadow-lg i.text-green-600,
    div.bg-white.p-6.rounded-2xl.shadow-lg i.text-purple-600 {
        color: #1B3C53 !important;
    }

    /* Card Status Badges */
    div.bg-white.p-6.rounded-2xl.shadow-lg span.bg-blue-100,
    div.bg-white.p-6.rounded-2xl.shadow-lg span.bg-gray-100 {
    ...
    div.bg-white.p-6.rounded-2xl.shadow-lg span.bg-yellow-100 {
        background: rgba(245, 158, 11, 0.08) !important;
        color: #d97706 !important;
        border: 1px solid rgba(245, 158, 11, 0.2) !important;
    }
    div.bg-white.p-6.rounded-2xl.shadow-lg span.bg-green-100 {
        background: rgba(16, 185, 129, 0.08) !important;
        color: #059669 !important;
        border: 1px solid rgba(16, 185, 129, 0.2) !important;
    }

    /* Card Text Highlights */
    div.bg-white.p-6.rounded-2xl.shadow-lg .text-blue-600 {
        color: #234C6A !important;
    }
    div.bg-white.p-6.rounded-2xl.shadow-lg .text-green-600:not(i):not(div) {
        color: #234C6A !important;
    }
    div.bg-white.p-6.rounded-2xl.shadow-lg .font-bold.text-green-600 {
        color: #234C6A !important;
        font-size: 0.95rem !important;
    }

    /* Progress Bars track & fill in Summary Card */
    div.bg-white.p-6.rounded-2xl.shadow-lg .bg-gray-200 {
        background-color: rgba(210, 193, 182, 0.35) !important;
        border: 1px solid rgba(69, 104, 130, 0.1) !important;
        height: 10px !important;
        border-radius: 9999px !important;
        overflow: hidden !important;
    }
    div.bg-white.p-6.rounded-2xl.shadow-lg .bg-green-600 {
        background: linear-gradient(90deg, #1B3C53 0%, #456882 100%) !important;
        height: 10px !important;
        border-radius: 9999px !important;
    }

    /* Compare Batches list & view badges */
    div.bg-white.p-6.rounded-2xl.shadow-lg .hover\:bg-gray-50:hover {
        background-color: rgba(27, 60, 83, 0.04) !important;
    }
    div.bg-white.p-6.rounded-2xl.shadow-lg .text-gray-400 {
        color: #456882 !important;
    }
    div.bg-white.p-6.rounded-2xl.shadow-lg .space-x-2 .text-green-600 {
    ...
    div.bg-white.p-6.rounded-2xl.shadow-lg .space-x-2 .text-red-600 {
    ...
    div.bg-white.p-6.rounded-2xl.shadow-lg .space-x-2 a.text-indigo-600 {
        background: rgba(27, 60, 83, 0.08) !important;
        color: #1B3C53 !important;
    ...
    div.bg-white.p-6.rounded-2xl.shadow-lg .space-x-2 a.text-indigo-600:hover {
        background: linear-gradient(135deg, #1B3C53, #456882) !important;
        color: #fff !important;
        box-shadow: 0 2px 8px rgba(27, 60, 83, 0.2) !important;
        transform: translateY(-1px) !important;
    }

    /* Batch Selection Tabs */
    .mb-6.glass-card .flex.flex-wrap.gap-2 a:not([class*="bg-indigo-600"]) {
        background: rgba(255, 255, 255, 0.5) !important;
        border: 1px solid rgba(69, 104, 130, 0.2) !important;
        color: #1f2937 !important;
        backdrop-filter: blur(4px);
        transition: all 0.3s cubic-bezier(0.4, 0, 0.2, 1) !important;
        box-shadow: 0 2px 6px rgba(27, 60, 83, 0.04) !important;
    }
    .mb-6.glass-card .flex.flex-wrap.gap-2 a:not([class*="bg-indigo-600"]) i {
        color: #4b5563 !important;
    }
    .mb-6.glass-card .flex.flex-wrap.gap-2 a:not([class*="bg-indigo-600"]):hover {
        background: rgba(27, 60, 83, 0.08) !important;
        border-color: rgba(27, 60, 83, 0.3) !important;
        color: #1B3C53 !important;
        transform: translateY(-2px) !important;
        box-shadow: 0 4px 10px rgba(27, 60, 83, 0.08) !important;
    }
    .mb-6.glass-card .flex.flex-wrap.gap-2 a:not([class*="bg-indigo-600"]):hover i {
        color: #1B3C53 !important;
    }
    .mb-6.glass-card .flex.flex-wrap.gap-2 a[class*="bg-indigo-600"] {
    ...
    .mb-6.glass-card .flex.flex-wrap.gap-2 a[class*="bg-indigo-600"] i,
    .mb-6.glass-card .flex.flex-wrap.gap-2 a[class*="bg-indigo-600"] div,
    .mb-6.glass-card .flex.flex-wrap.gap-2 a[class*="bg-indigo-600"] span {
        color: #ffffff !important;
    }
    .mb-6.glass-card p.text-sm.text-gray-500.mt-2 {
    ...
    .mb-6.glass-card p.text-sm.text-gray-500.mt-2 span.text-indigo-600 {
        color: #1B3C53 !important;
        font-weight: 700 !important;
    }
    .mb-6.glass-card p.text-sm.text-gray-500.mt-2 span.bg-indigo-100 {
        background: rgba(210, 193, 182, 0.4) !important;
        color: #234C6A !important;
        border: 1px solid rgba(69, 104, 130, 0.25) !important;
        font-weight: 600 !important;
        padding: 2px 8px !important;
    ...
    .feedback-card:hover {
        ...
        box-shadow: 0 10px 25px rgba(27, 60, 83, 0.12) !important;
        transform: translateY(-3px) !important;
    }
    .feedback-card span.bg-indigo-50 {
        background: rgba(27, 60, 83, 0.08) !important;
        color: #1B3C53 !important;
        border: 1px solid rgba(27, 60, 83, 0.15) !important;
    }
    .feedback-card span.bg-green-100 {
        background: rgba(16, 185, 129, 0.1) !important;
        color: #059669 !important;
        border: 1px solid rgba(16, 185, 129, 0.2) !important;
        font-weight: 600 !important;
    }
    .feedback-card span.bg-red-100 {
        background: rgba(239, 68, 68, 0.1) !important;
        color: #dc2626 !important;
        border: 1px solid rgba(239, 68, 68, 0.2) !important;
        font-weight: 600 !important;
    }
    .feedback-card i.text-yellow-400 {
        color: #234C6A !important;
        text-shadow: 0 0 8px rgba(35, 76, 106, 0.4) !important;
    }
    .feedback-card .bg-gray-50,
    .feedback-card .bg-indigo-50 {
        background: rgba(27, 60, 83, 0.04) !important;
        border: 1px solid rgba(27, 60, 83, 0.08) !important;
        transition: all 0.2s ease !important;
    }
    .feedback-card .bg-gray-50:hover,
    .feedback-card .bg-indigo-50:hover {
        background: rgba(27, 60, 83, 0.08) !important;
        border-color: rgba(27, 60, 83, 0.2) !important;
    }
    .feedback-card div[class*="bg-indigo-50"][class*="border-indigo-100"] {
        background: linear-gradient(135deg, rgba(27, 60, 83, 0.05) 0%, rgba(210, 193, 182, 0.1) 100%) !important;
        border: 1px solid rgba(69, 104, 130, 0.25) !important;
        box-shadow: inset 0 2px 4px rgba(27, 60, 83, 0.03) !important;
    }

    /* All Batches Summary Table */
    thead.bg-gradient-to-r.from-blue-50.to-indigo-50,
    thead[class*="from-blue-50"][class*="to-indigo-50"] {
        background: linear-gradient(135deg, rgba(27, 60, 83, 0.12) 0%, rgba(210, 193, 182, 0.2) 100%) !important;
    }
    thead.bg-gradient-to-r.from-blue-50.to-indigo-50 th {
        color: #1B3C53 !important;
        font-weight: 700 !important;
    }
    tbody tr.hover\:bg-blue-50:hover {
        background-color: rgba(27, 60, 83, 0.04) !important;
    }
    tbody tr.bg-indigo-50 {
        background-color: rgba(210, 193, 182, 0.2) !important;
    }
    tbody td .bg-gray-200 {
        background-color: rgba(210, 193, 182, 0.35) !important;
        border: 1px solid rgba(69, 104, 130, 0.1) !important;
    }
    tbody td .bg-blue-600,
    tbody td .bg-green-600,
    tbody td .bg-purple-600 {
        background: linear-gradient(90deg, #1B3C53 0%, #456882 100%) !important;
    }
    ...
    div.bg-white.p-6.rounded-2xl.shadow-lg:hover {
        transform: translateY(-4px) scale(1.008) !important;
        box-shadow: 0 16px 36px rgba(27, 60, 83, 0.12) !important;
        border-color: rgba(69, 104, 130, 0.45) !important;
        background: rgba(255, 255, 255, 0.8) !important;
    }
    ...
    div.bg-white.p-6.rounded-2xl.shadow-lg .bg-green-100,
    div.bg-white.p-6.rounded-2xl.shadow-lg .bg-purple-100 {
        background: linear-gradient(135deg, rgba(27, 60, 83, 0.12) 0%, rgba(35, 76, 106, 0.12) 100%) !important;
        border-radius: 12px !important;
        padding: 10px !important;
        border: 1px solid rgba(69, 104, 130, 0.2) !important;
    }
    ...
    /* Progress Bars track & fill in Summary Card */
    div.bg-white.p-6.rounded-2xl.shadow-lg .bg-gray-200 {
        background-color: rgba(27, 60, 83, 0.08) !important;
        border: 1px solid rgba(27, 60, 83, 0.05) !important;
        height: 10px !important;
        border-radius: 9999px !important;
        overflow: hidden !important;
    }
    div.bg-white.p-6.rounded-2xl.shadow-lg .bg-green-600 {
        background: linear-gradient(90deg, #1B3C53 0%, #234C6A 100%) !important;
        height: 10px !important;
        border-radius: 9999px !important;
    }
    ...
    .mb-6.glass-card .flex.flex-wrap.gap-2 a:not([class*="bg-indigo-600"]) {
        ...
        backdrop-filter: blur(4px);
        transition: all 0.3s cubic-bezier(0.4, 0, 0.2, 1) !important;
        box-shadow: 0 2px 6px rgba(27, 60, 83, 0.03) !important;
    }
    .mb-6.glass-card .flex.flex-wrap.gap-2 a:not([class*="bg-indigo-600"]) i {
        color: #4b5563 !important; /* Gray icon */
    }
    .mb-6.glass-card .flex.flex-wrap.gap-2 a:not([class*="bg-indigo-600"]):hover {
        background: rgba(27, 60, 83, 0.08) !important;
    ...
    .mb-6.glass-card .flex.flex-wrap.gap-2 a[class*="bg-indigo-600"] div,
    .mb-6.glass-card .flex.flex-wrap.gap-2 a[class*="bg-indigo-600"] span {
        color: #ffffff !important; /* Force all active elements to white */
    }
    .mb-6.glass-card p.text-sm.text-gray-500.mt-2 {
        color: #6b7280 !important;
        font-weight: 500 !important;
    ...
    .mb-6.glass-card p.text-sm.text-gray-500.mt-2 span.bg-indigo-100 {
        ...
        color: #234C6A !important;
        border: 1px solid rgba(35, 76, 106, 0.25) !important;
        font-weight: 600 !important;
        padding: 2px 8px !important;
    }
    ...
    .feedback-card i.text-yellow-400 {
        color: #1B3C53 !important;
        text-shadow: 0 0 8px rgba(27, 60, 83, 0.4) !important;
    }
    .feedback-card .bg-gray-50,
    .feedback-card .bg-indigo-50 {
        background: rgba(27, 60, 83, 0.04) !important;
    ...
    .feedback-card div[class*="bg-indigo-50"][class*="border-indigo-100"] {
        background: linear-gradient(135deg, rgba(27, 60, 83, 0.06) 0%, rgba(35, 76, 106, 0.04) 100%) !important;
        border: 1px solid rgba(69, 104, 130, 0.3) !important;
        box-shadow: inset 0 2px 4px rgba(27, 60, 83, 0.03) !important;
    }
    ...
    thead.bg-gradient-to-r.from-blue-50.to-indigo-50,
    thead[class*="from-blue-50"][class*="to-indigo-50"] {
        background: linear-gradient(135deg, rgba(27, 60, 83, 0.15) 0%, rgba(35, 76, 106, 0.15) 100%) !important;
    }
    thead.bg-gradient-to-r.from-blue-50.to-indigo-50 th {
        color: #1B3C53 !important;
        font-weight: 700 !important;
    ...
    input:focus, select:focus, textarea:focus {
        border-color: #456882 !important;
        box-shadow: 0 0 0 3px rgba(69, 104, 130, 0.25) !important;
    }
</style>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Helper to update description badge styling
    function updateRatingDescStyle(descElement, rating) {
        if (!descElement) return;
        descElement.classList.remove('rating-badge-poor', 'rating-badge-average', 'rating-badge-good', 'opacity-0');
        descElement.classList.add('opacity-100');
        
        if (rating <= 2) {
            descElement.classList.add('rating-badge-poor');
        } else if (rating == 3) {
            descElement.classList.add('rating-badge-average');
        } else {
            descElement.classList.add('rating-badge-good');
        }
    }
        ...
        document.querySelectorAll('.star-rating').forEach(ratingDiv => {
            ...
            if (currentValue) {
                ...
                if (descElement) {
                    ...
                    updateRatingDescStyle(descElement, currentValue);
                }
            }
        });
    }
    ...
    document.querySelectorAll('.star-rating span').forEach(star => {
        star.addEventListener('click', function() {
            ...
            if (descElement) {
                ...
                updateRatingDescStyle(descElement, rating);
            }
        ...
        star.addEventListener('mouseout', function() {
            ...
            // Reset custom inline colors to let CSS classes take over
            ...
            if (currentValue) {
                ...
                if (descElement) {
                    ...
                    descElement.className = 'rating-description text-sm italic opacity-0 transition-opacity duration-300';
                }
            }
        });
    });
