...
$course_id_filter = isset($_GET['course_id']) ? $_GET['course_id'] : null;
...
try {
    ...
    $classWhereClause = "s.batch_id = ? AND s.schedule_date BETWEEN ? AND ?";
    ...
    if ($course_id_filter) {
        $classWhereClause .= " AND s.course_id = ?";
        $classParams[] = $course_id_filter;
    }

    // Get classes for the selected month
    $stmt = $conn->prepare("SELECT s.*, c.name as course_name FROM schedule s LEFT JOIN courses c ON s.course_id = c.id WHERE $classWhereClause ORDER BY s.schedule_date, s.start_time");
    ...
    $countClassQuery = "SELECT COUNT(*) as total FROM schedule WHERE batch_id = ?";
    $countClassParams = [$batch_id];
    if ($course_id_filter) {
        $countClassQuery .= " AND course_id = ?";
        $countClassParams[] = $course_id_filter;
    }
    $stmt = $conn->prepare($countClassQuery);
    $stmt->execute($countClassParams);
    ...
    $upcomingClassQuery = "SELECT COUNT(*) as total FROM schedule WHERE batch_id = ? AND schedule_date >= ?";
    $upcomingClassParams = [$batch_id, $today];
    if ($course_id_filter) {
        $upcomingClassQuery .= " AND course_id = ?";
        $upcomingClassParams[] = $course_id_filter;
    }
    $stmt = $conn->prepare($upcomingClassQuery);
    $stmt->execute($upcomingClassParams);
    ...
    foreach ($classes as $class) {
        ...
        $course_display = !empty($class['course_name']) ? htmlspecialchars($class['course_name']) . ' - ' : '';
        $allEvents[] = [
            'id' => $class['id'],
            'title' => $course_display . $class['topic'],
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_whole_schedule'])) {
        $stmt = $conn->prepare("DELETE FROM schedule WHERE batch_id = ?");
        $stmt->execute([$batch_id]);
        header("Location: schedule.php?batch_id=" . $batch_id . "&filter=" . $filter . "&month=" . $currentMonth . "&year=" . $currentYear . "&success=Whole+schedule+deleted+successfully");
        exit();
    }
...
function generateCalendarDays($year, $month, $eventsByDate) {
    ...
    for ($day = 1; $day <= $daysInMonth; $day++) {
        ...
        foreach ($dayEvents as $event) {
            if ($event['type'] === 'class') {
                $hasClass = true;
                if ($event['is_cancelled'] ?? false) $hasCancelled = true;
            }
            if ($event['type'] === 'exam') $hasExam = true;
            if ($event['type'] === 'workshop') $hasWorkshop = true;
            
            if (!$event['is_past'] && !($event['is_cancelled'] ?? false)) {
                $hasUpcoming = true;
            }
            if ($event['is_past']) {
                $hasPast = true;
...
<body class="bg-gray-50">
        ...
        <div class="max-w-7xl mx-auto">
            ...
            <div class="flex justify-between items-center mb-8">
                <div>
                    ...
                    <?php if ($course_id_filter): ?>
                    <p class="text-blue-600 font-medium mt-1">Filtered by Course ID: <?= htmlspecialchars($course_id_filter) ?></p>
                    <?php endif; ?>
                ...
                <div class="flex space-x-3">
                    ...
                    <button onclick="confirmDeleteWholeSchedule()" class="px-5 py-3 bg-red-600 text-white font-medium rounded-lg hover:bg-red-700 transition-all duration-300 flex items-center shadow-md hover:shadow-lg">
                        <i class="fas fa-trash-alt mr-2"></i> Delete Whole Schedule
                    </button>

                    <form id="deleteWholeScheduleForm" method="POST" class="hidden">
                        <input type="hidden" name="delete_whole_schedule" value="1">
                    </form>
                </div>
            </div>
                ...
                <div class="grid grid-cols-7 gap-2">
                    ...
                    <?php foreach ($calendarDays as $day): ?>
                        <div class="calendar-day bg-white p-3 border border-gray-200 rounded-lg 
                            ...
                            <?= $day['day'] === '' ? 'bg-gray-50 opacity-40 cursor-default pointer-events-none' : '' ?>"
                            <?= $day['day'] !== '' ? 'onclick="openAddScheduleModal(\'' . $day['date'] . '\')"' : '' ?>>
                            <?php if ($day['day'] !== ''): ?>
    ...
    <script>
        ...
        function confirmDeleteWholeSchedule() {
            if (confirm("Are you sure you want to delete the whole schedule for this batch? This action cannot be undone, and will delete all classes scheduled for this batch.")) {
                document.getElementById('deleteWholeScheduleForm').submit();
            }
        }
    </script>
