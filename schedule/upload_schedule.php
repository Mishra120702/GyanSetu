            ...
            if ($handle !== FALSE) {
                ...
                while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
                    if ($row_count == 0) {
                        ...
                        $expected_headers = ['course_name_or_id', 'course_id', 'date', 'start_time', 'end_time', 'topic', 'description'];
                        
                        if (count(array_intersect($expected_headers, $headers)) < 5) {
                            $message = "Invalid CSV format. Required columns: course_name_or_id (or course_id), date, start_time, end_time, topic (description is optional)";
                            $message_type = "error";
                            break;
                        }
                    } else {
                        ...
                        // Check required fields
                        $course_input = !empty($row_data['course_name_or_id']) ? $row_data['course_name_or_id'] : (!empty($row_data['course_id']) ? $row_data['course_id'] : '');
                        
                        if (empty($course_input)) {
                            $row_errors[] = "Course name or ID is required";
                        } else {
                            if (is_numeric($course_input)) {
                                $course_stmt = $conn->prepare("SELECT id FROM courses WHERE id = ?");
                            } else {
                                $course_stmt = $conn->prepare("SELECT id FROM courses WHERE name = ?");
                            }
                            $course_stmt->execute([$course_input]);
                            $course_row = $course_stmt->fetch(PDO::FETCH_ASSOC);
                            if ($course_row) {
                                $row_data['course_id'] = $course_row['id'];
                            } else {
                                $row_errors[] = "Course '" . htmlspecialchars($course_input) . "' not found";
                            }
                        }
                        ...
                        if (empty($row_data['date'])) {
                            ...
                            $parsed_date = strtotime(str_replace('/', '-', $row_data['date']));
                            if ($parsed_date !== false) {
                                $row_data['date'] = date('Y-m-d', $parsed_date);
                            } else {
                                $row_errors[] = "Invalid date format (use YYYY-MM-DD or DD-MM-YYYY)";
                            }
                        }
            ...
            try {
                foreach ($confirmed_data as $row) {
                    if ($row['is_valid']) {
                        ...
                        $check_stmt = $conn->prepare("SELECT id FROM schedule WHERE batch_id = ? AND course_id = ? AND schedule_date = ? AND start_time = ?");
                        $check_stmt->execute([$batch_id, $row['course_id'], $row['date'], $row['start_time']]);
                        
                        if (!$check_stmt->fetch()) {
                            $insert_stmt = $conn->prepare("INSERT INTO schedule (batch_id, course_id, schedule_date, start_time, end_time, topic, description, created_by, is_back_schedule) 
                                                          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
...
$sample_csv = "course_name_or_id,date,start_time,end_time,topic,description\n";
$sample_csv .= "PHP Mastery,2026-02-21,10:00,12:00,Introduction to PHP,Basic PHP concepts and syntax\n";
$sample_csv .= "2,2026-02-22,14:00,16:00,Normalization,MySQL fundamentals\n";
$sample_csv .= "JavaScript Basics,2026-02-23,09:00,11:00,Functions,Variables and functions\n";
?>
<!DOCTYPE html>
<html lang="en">
<head>
                            ...
                            <div class="space-y-4">
                                ...
                                <div class="grid grid-cols-2 gap-3">
                                    <div class="p-3 bg-gray-50 rounded-lg col-span-2">
                                        <span class="font-mono text-sm font-semibold text-red-600">course_name_or_id</span>
                                        <p class="text-xs text-gray-500 mt-1">Exact name OR ID of the course</p>
                                    </div>
                                ...
                                <?php foreach ($uploaded_data as $index => $row): ?>
                                    <tr class="table-row-animation <?= $row['is_valid'] ? 'valid-row' : 'invalid-row' ?>">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                                            <?= htmlspecialchars($row['course_name_or_id'] ?? '') ?>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
