...
$filter_batch_id = $_GET['batch_id'] ?? 'all';
$filter_course_id = $_GET['course_id'] ?? 'all';

if ($filter_batch_id !== 'all' && in_array($filter_batch_id, $assignedBatches)) {
    $assignedBatches = [$filter_batch_id];
}

$course_condition = "";
$course_params = [];
if ($filter_course_id !== 'all' && !empty($filter_course_id)) {
    $course_condition = " AND t.course_id = ?";
    $course_params[] = $filter_course_id;
}

$filter_category = $_GET['category'] ?? 'chapter_wise';
$category_condition = "";
if (!empty($filter_category)) {
    $category_condition = " AND t.test_category = ?";
    $course_params[] = $filter_category;
}
...
if (!empty($assignedBatches)) {
    ...
    $additionalBatchConditions = [];
    foreach ($assignedBatches as $b) {
        $additionalBatchConditions[] = "FIND_IN_SET(?, t.additional_batches)";
    }
    $additionalBatchSql = !empty($additionalBatchConditions) ? " OR " . implode(" OR ", $additionalBatchConditions) : "";
    
    $testQuery = "
        SELECT 
            t.*,
            c.name as course_name,
            mt.topic_name as chapter_name,
            ...
            CASE 
                WHEN t.start_date IS NOT NULL AND t.start_date > NOW() THEN 'upcoming'
                WHEN t.end_date IS NOT NULL AND t.end_date < NOW() THEN 'expired'
                WHEN (t.start_date IS NULL OR t.start_date <= NOW()) 
                     AND (t.end_date IS NULL OR t.end_date >= NOW()) THEN 'available'
                ELSE 'unavailable'
            END as availability_status,
            -- Calculate seconds until start (for countdown)
            CASE 
                WHEN t.start_date IS NOT NULL AND t.start_date > NOW() 
                THEN TIMESTAMPDIFF(SECOND, NOW(), t.start_date)
                ELSE NULL
            END as seconds_until_start,
            -- Calculate seconds remaining (for countdown)
            CASE 
                WHEN t.end_date IS NOT NULL AND t.end_date > NOW() 
                THEN TIMESTAMPDIFF(SECOND, NOW(), t.end_date)
                ELSE NULL
            END as seconds_remaining
        FROM tests t
        LEFT JOIN courses c ON t.course_id = c.id
        LEFT JOIN main_topics mt ON t.chapter_id = mt.id
        ...
        WHERE t.is_active = 1
        AND (t.batch_id IN ($placeholders) OR t.batch_id IS NULL OR t.batch_id = '' $additionalBatchSql)
        $course_condition
        $category_condition
    ...
    $params = array_merge([$student['student_id']], array_values($assignedBatches), array_values($assignedBatches), $course_params);
    ...
    $testQuery = "
        ...
        ORDER BY 
            CASE 
                WHEN t.start_date IS NOT NULL AND t.start_date > NOW() THEN 0
                WHEN t.start_date IS NULL OR t.start_date <= NOW() THEN 1
                ELSE 2
            END,
            t.start_date ASC,
            t.created_at DESC
    ...
    $testStmt = $db->prepare($testQuery);
    $testStmt->execute(array_merge([$student['student_id']], $course_params));
    $tests = $testStmt->fetchAll(PDO::FETCH_ASSOC);
}

// Get targeted tests for this student
$targeted_tests = [];
$targetTestStmt = $db->prepare("SELECT test_id FROM test_students WHERE student_id = ?");
$targetTestStmt->execute([$student['student_id']]);
$targeted_tests = $targetTestStmt->fetchAll(PDO::FETCH_COLUMN);

// Filter tests by student targeting
$temp_tests = [];
foreach ($tests as $t) {
    if ($t['assigned_to'] === 'specific' && !in_array($t['id'], $targeted_tests)) {
        continue;
    }
    $temp_tests[] = $t;
}
...
// Build query for recent test attempts
$recentQuery = "
...
";
$recentParams = [$student['student_id']];

if ($filter_batch_id !== 'all') {
    $recentQuery .= " AND (t.batch_id = ? OR t.batch_id IS NULL OR t.batch_id = '')";
    $recentParams[] = $filter_batch_id;
}

if ($filter_course_id !== 'all' && !empty($filter_course_id)) {
    $recentQuery .= " AND t.course_id = ?";
    $recentParams[] = $filter_course_id;
}

$recentQuery .= " ORDER BY ta.submitted_at DESC LIMIT 5";

$recentAttemptsStmt = $db->prepare($recentQuery);
$recentAttemptsStmt->execute($recentParams);
...
// Format datetime for display (already in IST)
function formatToIST($datetime) {
    if (!$datetime) return 'No time limit';
...
<body class="min-h-screen" style="background-color:#F7F5F3;">
    <!-- Animated Background -->
    <div class="fixed inset-0 overflow-hidden pointer-events-none">
        <div class="absolute top-0 left-0 w-96 h-96 rounded-full mix-blend-multiply filter blur-3xl opacity-10 animate-pulse" style="background-color:#D2C1B6;"></div>
        <div class="absolute bottom-0 right-0 w-96 h-96 rounded-full mix-blend-multiply filter blur-3xl opacity-10 animate-pulse" style="background-color:#456882;"></div>
        <div class="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-96 h-96 rounded-full mix-blend-multiply filter blur-3xl opacity-10 floating" style="background-color:#D2C1B6;"></div>
    </div>
        ...
        <div class="glass-card sticky top-0 z-50">
            <div class="flex items-center justify-between p-4 md:p-6">
                <div class="flex items-center space-x-4">
                    <button onclick="toggleSidebar()" class="md:hidden p-2 rounded-lg text-white" style="background: linear-gradient(to right, #1B3C53, #456882);">
                ...
                <div class="flex items-center space-x-4">
                    ...
                    <div class="relative group">
                        <div class="w-10 h-10 rounded-full flex items-center justify-center text-white shadow-lg cursor-pointer" style="background: linear-gradient(135deg, #1B3C53, #456882);">
            ...
            <div class="max-w-7xl mx-auto space-y-6">
                ...
                <div class="glass-card rounded-2xl p-6 md:p-8">
                    <div class="flex flex-col md:flex-row justify-between items-start md:items-center">
                        ...
                        <div class="mt-4 md:mt-0 text-white px-6 py-3 rounded-xl" style="background: linear-gradient(to right, #1B3C53, #456882);">
                ...
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                    <div class="glass-card p-6 rounded-xl">
                        <div class="flex items-center justify-between">
                            ...
                            <div class="w-12 h-12 rounded-full flex items-center justify-center" style="background:rgba(69,104,130,0.15);">
                                <i class="fas fa-file-alt text-xl" style="color:#456882;"></i>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="glass-card p-6 rounded-xl">
                        <div class="flex items-center justify-between">
                            <div>
                                ...
                                <p class="text-2xl font-bold" style="color: #234C6A;"><?= $availableTests ?></p>
                            </div>
                            <div class="w-12 h-12 rounded-full flex items-center justify-center" style="background: rgba(35, 76, 106, 0.12);">
                                <i class="fas fa-play-circle text-xl" style="color: #234C6A;"></i>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="glass-card p-6 rounded-xl">
                        <div class="flex items-center justify-between">
                            <div>
                                ...
                                <p class="text-2xl font-bold" style="color:#1B3C53;"><?= $attemptedTests ?></p>
                            </div>
                            <div class="w-12 h-12 rounded-full flex items-center justify-center" style="background:rgba(27,60,83,0.1);">
                                <i class="fas fa-check-circle text-xl" style="color:#1B3C53;"></i>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="glass-card p-6 rounded-xl">
                        <div class="flex items-center justify-between">
                            <div>
                                ...
                                <p class="text-2xl font-bold" style="color:#456882;"><?= round($avgScore, 1) ?>%</p>
                            </div>
                            <div class="w-12 h-12 rounded-full flex items-center justify-center" style="background:rgba(69,104,130,0.15);">
                                <i class="fas fa-chart-line text-xl" style="color:#456882;"></i>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="glass-card rounded-2xl p-6">
                    <div class="flex justify-between items-center mb-6">
                        <h3 class="text-lg font-bold text-gray-800">
                            <i class="fas fa-history mr-2" style="color:#456882;"></i>
                            Recent Performance
                        </h3>
                        <a href="test_history.php" class="text-sm" style="color:#234C6A;" onmouseover="this.style.color='#1B3C53'" onmouseout="this.style.color='#234C6A'">
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="flex items-center justify-between p-4 bg-gray-50 rounded-lg hover:bg-[#456882]/5 transition-colors">
                            <div class="flex items-center space-x-4">
                            ...
                            <a href="test_result.php?test_id=<?= $attempt['test_id'] ?>" 
                               class="px-4 py-2 text-white rounded-lg transition-colors" style="background:#234C6A;" onmouseover="this.style.background='#1B3C53'" onmouseout="this.style.background='#234C6A'">
                                View Result
                            </a>
                        </div>
                        <?php endforeach; ?>
                ...
                <div class="glass-card rounded-2xl p-6">
                    <div class="flex flex-wrap justify-between items-center mb-6">
                        <h3 class="text-lg font-bold text-gray-800">
                            <i class="fas fa-rocket mr-2" style="color:#456882;"></i>
                            Available Tests
                    ...
                    <div class="flex flex-wrap gap-3 mb-6 border-b border-gray-200 pb-4">
                        <a href="?category=chapter_wise<?= $filter_batch_id !== 'all' ? '&batch_id='.$filter_batch_id : '' ?><?= $filter_course_id !== 'all' ? '&course_id='.$filter_course_id : '' ?>" 
                           class="px-4 py-2 rounded-lg text-sm font-medium transition-all <?= ($filter_category === 'chapter_wise') ? 'text-white shadow-md' : 'bg-gray-100 text-gray-600 hover:bg-gray-200' ?>" <?= ($filter_category === 'chapter_wise') ? 'style="background:#456882;"' : '' ?>>
                           Chapterwise Tests
                        </a>
                        <a href="?category=weekly<?= $filter_batch_id !== 'all' ? '&batch_id='.$filter_batch_id : '' ?><?= $filter_course_id !== 'all' ? '&course_id='.$filter_course_id : '' ?>" 
                           class="px-4 py-2 rounded-lg text-sm font-medium transition-all <?= ($filter_category === 'weekly') ? 'text-white shadow-md' : 'bg-gray-100 text-gray-600 hover:bg-gray-200' ?>" <?= ($filter_category === 'weekly') ? 'style="background:#456882;"' : '' ?>>
                           Weekly Tests
                        </a>
                    </div>
                            ...
                            <div class="flex flex-col lg:flex-row lg:items-start justify-between">
                                ...
                                <div class="flex-1">
                                    <div class="flex items-start space-x-3 mb-4">
                                        <div class="w-10 h-10 rounded-lg flex items-center justify-center text-white font-bold" style="background: linear-gradient(135deg, #456882, #1B3C53);">
                                        ...
                                        <div class="flex-1">
                                            <div class="flex flex-wrap items-center gap-2 mb-1">
                                                ...
                                                <span class="px-2 py-1 rounded-lg text-xs font-medium" style="background:rgba(69,104,130,0.15); color:#234C6A;">
                                                    <?= $test['attempts_made'] ?>/<?= $test['max_attempts'] ?> Attempts
                                                </span>
                                                <?php endif; ?>
                                            </div>
                                            <div class="flex flex-wrap gap-2 mt-2">
                                                <?php if ($test['test_category'] === 'chapter_wise'): ?>
                                                <span class="inline-flex items-center px-2.5 py-1 rounded-lg text-xs font-medium border" style="background:rgba(27,60,83,0.08); color:#1B3C53; border-color:rgba(27,60,83,0.2);">
                                                    <i class="fas fa-book-open mr-1.5" style="color:#1B3C53;"></i> Chapterwise Test
                                                </span>
                                                <?php elseif ($test['test_category'] === 'weekly'): ?>
                                                <span class="inline-flex items-center px-2.5 py-1 rounded-lg text-xs font-medium bg-orange-50 text-orange-700 border border-orange-200">
                                                    <i class="fas fa-calendar-check mr-1.5 text-orange-500"></i> Weekly Test
                                                </span>
                                                <?php endif; ?>
                                                
                                                <?php if (!empty($test['chapter_name'])): ?>
                                                <span class="inline-flex items-center px-2.5 py-1 rounded-lg text-xs font-medium bg-emerald-50 text-emerald-700 border border-emerald-200">
                                                    <i class="fas fa-bookmark mr-1.5 text-emerald-500"></i> Chapter: <?= htmlspecialchars($test['chapter_name']) ?>
                                                </span>
                                                <?php endif; ?>
                                                ...
                                                <span class="inline-flex items-center px-2.5 py-1 rounded-lg text-xs font-medium bg-gray-100 text-gray-700 border border-gray-200">
                                                    <i class="fas fa-tag mr-1.5 text-gray-500"></i> Subject: <?= htmlspecialchars($test['subject']) ?>
                                                </span>
                                                <?php if (!empty($test['course_name'])): ?>
                                                <span class="inline-flex items-center px-2.5 py-1 rounded-lg text-xs font-medium border" style="background:rgba(69,104,130,0.1); color:#234C6A; border-color:rgba(69,104,130,0.2);">
                                                    <i class="fas fa-graduation-cap mr-1.5" style="color:#234C6A;"></i> Course: <?= htmlspecialchars($test['course_name']) ?>
                                                </span>
                                                <?php endif; ?>
                                                <span class="inline-flex items-center px-2.5 py-1 rounded-lg text-xs font-medium border" style="background:rgba(27,60,83,0.08); color:#1B3C53; border-color:rgba(27,60,83,0.15);">
                                                    <i class="fas fa-users mr-1.5" style="color:#1B3C53;"></i> Batch: <?= !empty($test['batch_id']) ? htmlspecialchars($test['batch_id']) : 'All Batches' ?>
                                                </span>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div class="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
                                        <div class="p-2 rounded-lg text-center" style="background:rgba(210,193,182,0.2);">
                                    ...
                                    <p class="text-sm text-gray-600 p-3 rounded-lg" style="background:rgba(210,193,182,0.2);">
                                ...
                                <div class="mt-6 lg:mt-0 lg:ml-6 flex flex-col items-center lg:items-end space-y-3">
                                    ...
                                    <div class="flex flex-col space-y-2">
                                        ...
                                        <a href="take_test.php?test_id=<?= $test['id'] ?>" 
                                           class="px-6 py-3 text-white rounded-xl transition-all text-center font-medium" style="background: linear-gradient(to right, #1B3C53, #456882);" onmouseover="this.style.background='linear-gradient(to right, #234C6A, #1B3C53)'" onmouseout="this.style.background='linear-gradient(to right, #1B3C53, #456882)'">
                                        ...
                                        <a href="test_result.php?test_id=<?= $test['id'] ?>&attempt_id=<?= $test['last_attempt_id'] ?>" 
                                           class="px-6 py-3 text-white rounded-xl transition-all text-center font-medium" style="background:#456882;" onmouseover="this.style.background='#1B3C53'" onmouseout="this.style.background='#456882'">
