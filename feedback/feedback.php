...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit_feedback'])) {
    ...
    $stmt = $db->prepare("INSERT INTO feedback (date, student_name, email, is_regular, batch_id, course_name, 
                         ...
                         suggestions, feedback_text, show_to_trainer) 
                         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 1)");
    $stmt->execute([
        date('Y-m-d'),
        $_POST['student_name'],
    ...
    logSystemActivity($db, $user_id, 'FEEDBACK_SUBMITTED', "Feedback submitted by student {$_POST['student_name']} (Email: {$_POST['email']}) for course {$_POST['course_name']}, Batch: {$_POST['batch_id']}.");
    $success = true;
}
...
function getPaginationHtml($current_page, $total_pages, $base_url, $params) {
    ...
    $html = '<div class="flex items-center justify-between mt-6 pt-4 border-t border-gray-200">';
    ...
    if ($current_page > 1) {
        ...
        $html .= '<a href="?' . http_build_query($params) . '" class="px-3 py-1 bg-gray-100 hover:bg-gray-200 rounded-lg transition-colors text-gray-700 hover:text-gray-900">';
    ...
    $html .= '<select onchange="changePerPage(this.value)" class="px-2 py-1 border rounded-lg text-sm bg-white">';
...
<head>
    ...
    <style>
        ...
        .glass-card:hover {
            box-shadow: 0 8px 36px rgba(27,60,83,0.11);
        }
        ...
        .stat-card:hover .stat-icon { transform: scale(1.06) rotate(-1deg); }
        ...
        /* Icon variants */
        .stat-total .stat-icon { background: linear-gradient(135deg, var(--navy-deep), var(--navy-light)); }
        .stat-rating .stat-icon { background: linear-gradient(135deg, #f59e0b, #fbbf24); }
        .stat-satisfied .stat-icon { background: linear-gradient(135deg, #10b981, #34d399); }
        .stat-action .stat-icon { background: linear-gradient(135deg, #8b5cf6, #a78bfa); }
        .stat-trainer .stat-icon { background: linear-gradient(135deg, #ec4899, #f472b6); }
        ...
        .tab-nav {
            ...
            padding: 4px;
            margin-bottom: 2rem;
            box-shadow: 0 2px 12px rgba(27,60,83,0.06);
        ...
        .filter-panel h3 i { transition: transform 0.3s ease; }
        .filter-panel.collapsed h3 i { transform: rotate(-90deg); }
        ...
        .data-table {
            ...
            border-radius: 12px;
            overflow: hidden;
            box-shadow: 0 2px 12px rgba(27,60,83,0.05);
        }
        .data-table thead th {
        ...
        .data-table tbody td {
            padding: 0.9rem 1rem;
            border-bottom: 1px solid rgba(210,193,182,0.25);
        ...
        input:checked + .toggle-slider { background-color: var(--navy-light); }
        ...
        .stars-inline { font-size: 14px; letter-spacing: 2px; }
    ...
    <div class="ml-0 lg:ml-64 transition-all duration-300 min-h-screen">
        ...
        <header class="bg-white shadow-sm sticky top-0 z-40 border-b border-gray-200">
            <div class="px-6 py-4 flex justify-between items-center">
                <div>
                    <h1 class="text-2xl font-bold text-[#1B3C53]">
                        Feedback System
                    </h1>
                    <p class="text-gray-600 text-sm">Manage student and weekly feedback</p>
                </div>
                <div class="flex items-center space-x-3">
                    <div class="w-10 h-10 bg-gradient-to-r from-[#1B3C53] to-[#456882] rounded-full flex items-center justify-center text-white font-bold shadow-md">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4 mb-6">
                    <div class="stat-card stat-total">
                        ...
                        <div class="stat-content">
                            ...
                            <div class="stat-number"><?= $summary['total_feedback'] ?? 0 ?></div>
                    ...
                    <div class="stat-card stat-rating">
                        ...
                        <div class="stat-content">
                            ...
                            <div class="stat-number"><?= number_format($summary['avg_class_rating'] ?? 0, 1) ?></div>
                            <div class="stat-sub">/ 5.0</div>
                        </div>
                    </div>
                    
                    <div class="stat-card stat-satisfied">
                        <div class="stat-icon"><i class="fas fa-smile"></i></div>
                        <div class="stat-content">
                            <div class="stat-label">Satisfaction Rate</div>
                            <div class="stat-number"><?= $summary['total_feedback'] > 0 ? round(($summary['satisfied_count'] / $summary['total_feedback']) * 100) : 0 ?>%</div>
                            <div class="stat-sub"><?= $summary['satisfied_count'] ?? 0 ?> satisfied</div>
                        </div>
                    </div>
                    ...
                    <div class="stat-card stat-action">
                        ...
                        <div class="stat-content">
                            ...
                            <div class="stat-number"><?= $summary['action_taken_count'] ?? 0 ?></div>
                            <div class="stat-sub">Resolved</div>
                        </div>
                    </div>
                    
                    <div class="stat-card stat-trainer">
                        <div class="stat-icon"><i class="fas fa-chalkboard-teacher"></i></div>
                        <div class="stat-content">
                            <div class="stat-label">Show to Trainers</div>
                            <div class="stat-number"><?= $summary['show_to_trainer_count'] ?? 0 ?></div>
                            <div class="stat-sub">Visible to trainers</div>
                        </div>
                    </div>
                </div>
                ...
                <div class="glass-card p-6">
                    <div class="flex justify-between items-center mb-6 flex-wrap gap-4">
                        <h2 class="text-xl font-bold text-[#1B3C53]">
                            <i class="fas fa-table mr-2 text-[#456882]"></i>
                            Student Feedback Records
                        </h2>
                        <div class="flex gap-2">
                            <button class="px-4 py-2 bg-gray-100 hover:bg-gray-200 rounded-lg transition-colors text-sm font-semibold text-gray-700" onclick="toggleFilter('student')">
                                <i class="fas fa-filter mr-2"></i> Filter
                            </button>
                            <a href="?tab=student_feedback" class="px-4 py-2 bg-gradient-to-r from-[#1B3C53] to-[#234C6A] text-white rounded-lg hover:opacity-90 transition text-sm font-semibold shadow-sm">
                        ...
                        <div class="filter-content mt-4">
                            <form method="GET" action="" id="filterForm">
                                ...
                                <div class="flex justify-between items-center mt-4">
                                    <div class="flex gap-2">
                                        <button type="submit" class="px-4 py-2 bg-gradient-to-r from-[#1B3C53] to-[#234C6A] text-white rounded-lg hover:opacity-90 transition font-semibold text-sm shadow-sm">
                                            <i class="fas fa-filter mr-2"></i> Apply Filters
                                        </button>
                                        <a href="feedback.php?tab=student_feedback" class="px-4 py-2 bg-gray-100 hover:bg-gray-200 rounded-lg transition text-sm font-semibold text-gray-700">
                    ...
                    <div class="bulk-actions-bar" id="bulkActionsBar">
                        ...
                        <div class="flex gap-2">
                            <select id="bulkShowToTrainer" class="px-3 py-1 rounded-lg text-gray-700 bg-white border border-gray-300 text-sm font-medium">
                                <option value="1">Show to Trainers</option>
                                <option value="0">Hide from Trainers</option>
                            </select>
                            <button onclick="applyBulkAction()" class="px-4 py-1 bg-white text-[#1B3C53] rounded-lg hover:bg-gray-100 transition font-semibold text-sm">
                                Apply
                            </button>
                            <button onclick="clearSelection()" class="px-4 py-1 bg-gray-200 text-gray-700 rounded-lg hover:bg-gray-300 transition font-semibold text-sm">
                                Cancel
                            </button>
                        </div>
                    </div>
                        ...
                        <table class="data-table">
                            ...
                            <tbody>
                                ...
                                <tr class="expand-row" id="row-<?= $item['id']; ?>" onclick="toggleRowDetails(<?= $item['id']; ?>)">
                                    ...
                                    <td>
                                        <div class="flex items-center">
                                            <div class="avatar bg-gradient-to-r from-[#456882] to-[#234C6A] mr-3">
                                                <?= $initials ?>
                                            </div>
                                            <div>
            ...
            <div id="weekly_feedback_tab" class="tab-content <?= $active_tab === 'weekly_feedback' ? 'active' : '' ?>">
                <div class="glass-card p-6">
                    <div class="flex justify-between items-center mb-6">
                        <h2 class="text-xl font-bold text-[#1B3C53]">
                            <i class="fas fa-calendar-week mr-2 text-[#456882]"></i>
                            Weekly Feedback Records
                        </h2>
                        <div class="flex gap-2">
                            <button class="px-4 py-2 bg-gray-100 hover:bg-gray-200 rounded-lg transition-colors text-sm font-semibold text-gray-700" onclick="toggleFilter('weekly')">
                                <i class="fas fa-filter mr-2"></i> Filter
                            </button>
                            <a href="?tab=weekly_feedback" class="px-4 py-2 bg-gradient-to-r from-[#1B3C53] to-[#234C6A] text-white rounded-lg hover:opacity-90 transition text-sm font-semibold shadow-sm">
                                <i class="fas fa-redo mr-2"></i> Reset
                            </a>
                        </div>
                    </div>
                            ...
                            <form method="GET" action="">
                                ...
                                <div class="flex justify-between items-center mt-4">
                                    <div class="flex gap-2">
                                        ...
                                        <a href="feedback.php?tab=weekly_feedback" class="px-4 py-2 bg-gray-100 hover:bg-gray-200 rounded-lg transition text-sm font-semibold text-gray-700">
                                            <i class="fas fa-redo mr-2"></i> Reset All
                                        </a>
                                    </div>
                                    <div class="text-sm text-gray-500">
