...
$course_cond = !empty($course_id) ? " AND course_id = ?" : "";
$course_col = !empty($course_id) ? ", course_id" : "";
$course_val = !empty($course_id) ? ", ?" : "";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    elseif (isset($_POST['edit_main_topic'])) {
        ...
        $stmt = $db->prepare("UPDATE main_topics SET chapter = ?, topic_name = ?, topic_type = ? WHERE id = ? AND batch_name = ?" . $course_cond);
        $params = [$chapter, $topic_name, $topic_type, $main_topic_id, $batch_id];
        if (!empty($course_id)) $params[] = $course_id;
        $stmt->execute($params);
    ...
    elseif (isset($_POST['delete_main_topic'])) {
        ...
        $stmt = $db->prepare("DELETE FROM main_topics WHERE id = ? AND batch_name = ?" . $course_cond);
        $params = [$main_topic_id, $batch_id];
        if (!empty($course_id)) $params[] = $course_id;
        $stmt->execute($params);
    ...
    elseif (isset($_POST['toggle_main_topic'])) {
        ...
        $stmt = $db->prepare("UPDATE main_topics SET is_active = NOT is_active WHERE id = ? AND batch_name = ?" . $course_cond);
        $params = [$main_topic_id, $batch_id];
        if (!empty($course_id)) $params[] = $course_id;
        $stmt->execute($params);
    ...
    $redirect_url = "progress_batch.php?batch_id=" . urlencode($batch_id);
    if (!empty($course_id)) {
        $redirect_url .= "&course_id=" . urlencode($course_id);
    }
...
$main_topics_stmt = $db->prepare("SELECT * FROM main_topics WHERE batch_name = ?" . $course_cond . " AND is_active = 1 ORDER BY chapter");
$params = [$batch_id];
if (!empty($course_id)) $params[] = $course_id;
$main_topics_stmt->execute($params);
