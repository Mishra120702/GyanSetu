...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_bio'])) {
        ...
        $update_stmt = $conn->prepare("UPDATE students SET bio = ? WHERE student_id = ?");
        $update_stmt->execute([$new_bio, $student_id]);
        $student['bio'] = $new_bio; // Update the local array
        $success_message = "Student bio updated successfully!";
    }
...
<body>
                ...
                <div class="lg:col-span-1">
                    ...
                    <div class="stats-card mt-6">
                        <div class="flex justify-between items-center mb-3">
                            <h3 class="font-bold text-gray-800"><i class="fas fa-comment-dots text-blue-500 mr-2"></i>Bio / Remarks</h3>
                            <button type="button" class="btn btn-sm btn-outline-primary" data-bs-toggle="modal" data-bs-target="#editBioModal">
                                <i class="fas fa-edit"></i> Edit
                            </button>
                        </div>
                        <div class="text-gray-600 text-sm whitespace-pre-wrap"><?= !empty($student['bio']) ? htmlspecialchars($student['bio']) : 'No remarks added yet.' ?></div>
                    </div>
                </div>
    ...
    <div class="modal fade" id="transferStudentModal" tabindex="-1" aria-labelledby="transferStudentModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <form id="transferForm" action="transfer_student.php" method="POST">
                    <div class="modal-body">
                        <input type="hidden" name="student_id" value="<?= $student['student_id'] ?>">
                        <input type="hidden" name="from_batch_id" value="<?= htmlspecialchars($batch['batch_id'] ?? '') ?>">
                        <div class="mb-3">
    ...
    <div class="modal fade" id="editBioModal" tabindex="-1" aria-labelledby="editBioModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="editBioModalLabel">Edit Student Bio / Remarks</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <form action="" method="POST">
                    <div class="modal-body">
                        ...
                        <div class="mb-3">
                            <label for="bio" class="form-label">Progress / Remarks</label>
                            <textarea class="form-control" id="bio" name="bio" rows="6" placeholder="Enter remarks or progress about the student..."><?= htmlspecialchars($student['bio'] ?? '') ?></textarea>
                        </div>
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button>
        ...
        document.getElementById('transferForm').addEventListener('submit', function(e) {
            ...
            .then(data => {
                ...
                if(data.success) {
                    ...
                    alert(data.message || 'Error transferring student');
                }
            })
            .catch(err => {
                btn.innerHTML = originalText;
