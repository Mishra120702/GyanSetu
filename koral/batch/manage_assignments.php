        ...
        if (!isset($_POST['csrf_token']) || $_POST['csrf_token'] !== $_SESSION['csrf_token']) {
            ...
            if ($upload_ok) {
                ...
                try {
                    ...
                    $stmt_ins = $db->prepare("
                        INSERT INTO uploads (title, description, file_path, file_type, due_date, due_time, max_marks, uploaded_by, course_id, uploaded_at)
                        VALUES (:title, :description, :file_path, 'Assignment', :due_date, :due_time, :max_marks, :uploaded_by, :course_id, NOW())
                    ");
                    $stmt_ins->execute([
                        ':title' => $title,
