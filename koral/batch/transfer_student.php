...
try {
    ...
    $current_batch = $_POST['current_batch'] ?? null;
    ...
    try {
        // Get student details
        $studentStmt = $db->prepare("SELECT first_name, last_name, email, user_id FROM students WHERE student_id = ? AND batch_name = ?");
        $studentStmt->execute([$student_id, $current_batch]);
        ...
        $updateStmt = $db->prepare("UPDATE students SET batch_name = ?, current_status = 'active' WHERE student_id = ? AND batch_name = ?");
