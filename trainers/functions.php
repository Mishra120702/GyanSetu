...
function getFilteredTrainers($filters, $limit = 20, $offset = 0) {
    ...
    switch ($filters['experience']) {
        case '1-3':
            $where[] = "t.years_of_experience BETWEEN 1 AND 3";
            break;

        case '4-6':
            $where[] = "t.years_of_experience BETWEEN 4 AND 6";
            break;

        case '7+':
            $where[] = "t.years_of_experience >= 7";
            break;
    }
}
    ...
    switch ($filters['rating']) {
        ...
        case '4+':
            $where[] = "(SELECT AVG(f.rating)
                         FROM feedback f
                         JOIN batches b2 ON f.batch_id = b2.batch_id
                         WHERE b2.batch_mentor_id = t.id) >= 4";
            break;
        ...
        case '3-4':
            $where[] = "(SELECT AVG(f.rating)
                         FROM feedback f
                         JOIN batches b2 ON f.batch_id = b2.batch_id
                         WHERE b2.batch_mentor_id = t.id) BETWEEN 3 AND 4";
            break;
        ...
        case '1-3':
            $where[] = "(SELECT AVG(f.rating)
                         FROM feedback f
                         JOIN batches b2 ON f.batch_id = b2.batch_id
                         WHERE b2.batch_mentor_id = t.id) BETWEEN 1 AND 3";
            break;
    }
}
    if (!empty($where)) {
